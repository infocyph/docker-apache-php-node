...
LABEL org.opencontainers.image.source="https://github.com/infocyph/docker-apache-php-node"
...
RUN set -eux; \
    ...
    echo "ServerName ${BANNER_TITLE,,}" >> /etc/apache2/apache2.conf && \
...
RUN curl -fsSL https://raw.githubusercontent.com/ohmybash/oh-my-bash/master/tools/install.sh | bash -s -- --unattended && \
    ...
    echo 'show-banner "${BANNER_TITLE}" "Container: PHP ${PHP_VERSION} with Apache"' >> ~/.bashrc
WORKDIR /app
